...
exports.archiveLayout = function (config, archiveId, layout, callback) {
  if (typeof callback !== 'function') {
  ...
  api(config, 'POST', '/archive/' + encodeURIComponent(archiveId) + '/layout', layout,
    function archiveLayoutCallback(err, response, body) {
      if (err) {
        callback(err);
      } else if (response.statusCode !== 200) {
        if (response && response.statusCode === 404) {
